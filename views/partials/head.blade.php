...
@foreach(config('twill.global_css') as $css)
    <link href="{{ $css }}" rel="stylesheet" crossorigin/>
@endforeach

@foreach(config('twill.global_js_head') as $globalJsHead)
    <script src="{{ $globalJsHead }}"></script>
@endforeach
